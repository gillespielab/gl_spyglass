...
def make_nwbs(data_path, animal, dates=None, excluded_dates=[], dry_run=True):
    ...
    for d in date_list: # create nwb for each date
        ...
        try:
            ...
            try:
                os.rename(
                    OUTPUT_PATH + inspector_file,
                    LOGS_PATH + inspector_file
                )
...
def main():
    ...
    parser.add_argument("name", type=str, help='Name of subject whose metadata is being updated')
    parser.add_argument("--dates", type=int, nargs="+", help='Dates of raw data to be converted. If unspecified, all dates will be converted to NWB')
    parser.add_argument("--excluded", action='append', help='Dates of raw data files to be excluded from NWB conversion.')
